...
class ViewListKendaraan extends HtmlResponse{
	...
	function ParseTemplate($data=NULL){
		$this->mrTemplate->Addvar('content', 'URL_ADD', Dispatcher::Instance()->GetUrl('Kendaraan', 'AddKendaraan', 'View', 'html'));
			if (is_array($data))
            extract($data);
        	if (!empty($message)) {
        ...
        if (!empty($data)) {
            ...
            foreach ($data as $val) {
				...
				$val['URL_UPDATE'] = Dispatcher::Instance()->GetUrl('Kendaraan', 'UpdateKendaraan', 'view', 'html').'&kodeAset='.Dispatcher::Instance()->Encrypt($val['KODE_ASET']);
				//delete
				$label = "Kendaraan";
				$idEnc = Dispatcher::Instance()->Encrypt($val['KODE_ASET']);
				$dataName = Dispatcher::Instance()->Encrypt($val['NAMA_ASET']);
                $dataName = Dispatcher::Instance()->Encrypt($val['PIC']);
                $dataName = Dispatcher::Instance()->Encrypt($val['DESKRIPSI_ASET']);
				$urlAccept = 'Kendaraan|DeleteKendaraan|do|json';
				$urlReturn = 'Kendaraan|ListKendaraan|view|html';
