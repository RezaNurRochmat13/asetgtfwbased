...
class ViewListHardware extends HtmlResponse{
	...
	function ParseTemplate($data=NULL){
		$this->mrTemplate->Addvar('content', 'URL_ADD', Dispatcher::Instance()->GetUrl('Hardware', 'AddHardware', 'View', 'html'));
			if (is_array($data))
            extract($data);
        	if (!empty($message)) {
        ...
        if (!empty($data)) {
            ...
            foreach ($data as $val) {
				...
				$val['URL_UPDATE'] = Dispatcher::Instance()->GetUrl('Hardware', 'UpdateHardware', 'view', 'html').'&kodeAset='.Dispatcher::Instance()->Encrypt($val['KODE_ASET']);
				//delete
				$label = "Hardware";
				...
				$urlAccept = 'Hardware|DeleteHardware|do|json';
				$urlReturn = 'Hardware|ListHardware|view|html';
