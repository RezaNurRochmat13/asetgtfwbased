...
class ViewListSoftware extends HtmlResponse{
	...
	function ParseTemplate($data=NULL){
		$this->mrTemplate->Addvar('content', 'URL_ADD', Dispatcher::Instance()->GetUrl('Software', 'AddSoftware', 'View', 'html'));
			if (is_array($data))
            extract($data);
        	if (!empty($message)) {
        ...
        if (!empty($data)) {
            ...
            foreach ($data as $val) {
				...
				$val['URL_UPDATE'] = Dispatcher::Instance()->GetUrl('Software', 'UpdateSoftware', 'view', 'html').'&kodeAset='.Dispatcher::Instance()->Encrypt($val['KODE_ASET']);
				//delete
				$label = "Software";
				...
				$urlAccept = 'Software|DeleteSoftware|do|json';
				$urlReturn = 'Software|ListSoftware|view|html';
