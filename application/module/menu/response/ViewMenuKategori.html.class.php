...
class ViewMenuKategori extends HtmlResponse {
         ...
         if ($len>0){
            ...
            for ($i=0; $i<$len; $i++) {
					if($menuName != $data[$i]['MenuName']){
						...
						$this->mrTemplate->clearTemplate('sub_nav_kategori');
						$this->mrTemplate->clearTemplate('sub_nav_item_kategori');
						
						for ($j=0; $j<$len; $j++) {
							
							if($menuName == $data[$j]['MenuName']){
								
									$this->mrTemplate->setAttribute('sub_nav_kategori', 'visibility', 'visible');
									
									$this->mrTemplate->addVar('sub_nav_kategori', 'UL_ID', $data[$j]['MenuId']);

									$this->mrTemplate->addVar('sub_nav_item_kategori', 'LEFT_SUB_NAV_VALUE', $data[$j]['subMenu']);
									$url = Dispatcher::Instance()->GetUrl($data[$j]['subMenuModule'],$data[$j]['subMenuSubModule'],$data[$j]['subMenuAction'],$data[$j]['subMenuType']);
									$this->mrTemplate->addVar('sub_nav_item_kategori', 'LEFT_SUB_NAV', $url);
									$this->mrTemplate->parseTemplate('sub_nav_item_kategori', 'a');
